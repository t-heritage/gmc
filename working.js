  ...
  scheduleLists.forEach(list => {
    ...
    if (filter === 'completed') {
      ...
      for (const itemId in completedItems) {
        // Debug output
        console.log(`Processing completed item: ${itemId}`);
        
        // Get completions array for this item
        const completionEntries = completedItems[itemId];
        
        if (Array.isArray(completionEntries) && completionEntries.length > 0) {
          // Debug output
          console.log(`Found ${completionEntries.length} completion entries for item ${itemId}`);
          ...
          completionEntries.forEach((completion, index) => {
            ...
            if (titleElement) {
              // First try to get item name using the helper function
              const itemTitle = getItemNameBySlug(itemId);
              
              // If helper function returns default value, try alternative approaches
              if (itemTitle === "Scheduled Item") {
                // Try to find title in other ways:
                // 1. Check if it's stored in completion object
                if (completion.itemTitle) {
                  titleElement.textContent = completion.itemTitle;
                } else {
                  // 2. Fall back to using itemId with formatting
                  titleElement.textContent = `Activity ${itemId.replace(/-/g, ' ')}`;
                }
              } else {
            ...
            if (dateElement && completion.completedDate) {
              ...
              console.log(`Set completion date: ${dateElement.textContent}`);
            }
            ...
            if (ratingElement && completion.rating) {
              ...
              console.log(`Set rating: ${ratingElement.textContent}`);
            }
            ...
            if (notesElement && completion.notes) {
              ...
              console.log(`Set notes: ${completion.notes.substring(0, 20)}...`);
            }
      ...
      // Existing code for non-completed filters
      let hasVisibleItems = false;
